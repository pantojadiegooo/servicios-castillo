...
function configuration(mode = 'live') {
  ...
  return {
    ...
    PUBLIC_CARE_BASIC_PAYMENT_LINK: `https://buy.stripe.com/${prefix}BasicFixture`,
    PUBLIC_CARE_PRO_PAYMENT_LINK: `https://buy.stripe.com/${prefix}ProFixture`,
    PUBLIC_CARE_ENTERPRISE_PAYMENT_LINK: `https://buy.stripe.com/${prefix}EnterpriseFixture`,
    PUBLIC_CARE_CUSTOMER_PORTAL_URL: `https://billing.stripe.com/p/login/${prefix}PortalFixture`,
  };
}
...
test('live checkout maps all three plans and the customer portal independently', () => {
  ...
  assert.equal(result.plans.basic.href, env.PUBLIC_CARE_BASIC_PAYMENT_LINK);
  assert.equal(result.plans.pro.href, env.PUBLIC_CARE_PRO_PAYMENT_LINK);
  assert.equal(result.plans.enterprise.href, env.PUBLIC_CARE_ENTERPRISE_PAYMENT_LINK);
  assert.equal(result.portalUrl, env.PUBLIC_CARE_CUSTOMER_PORTAL_URL);
});

test('all links and the portal are mandatory when enabling checkout', () => {
  for (const key of Object.keys(configuration()).filter(key => /LINK|PORTAL/.test(key))) {
    const env = configuration();
    delete env[key];
...
test('rejects unsafe or noncanonical payment destinations without echoing their values', () => {
  for (const url of [
    ...
    const env = { ...configuration(), PUBLIC_CARE_BASIC_PAYMENT_LINK: url };
    assert.throws(() => getCareCheckoutConfig(env), error => {
      assert.ok(!error.message.includes(url));
      return /PUBLIC_CARE_BASIC_PAYMENT_LINK/.test(error.message);
    });
  }
});
...
test('rejects wrong-host portal and duplicate plan links', () => {
  assert.throws(() => getCareCheckoutConfig({
    ...configuration(), PUBLIC_CARE_CUSTOMER_PORTAL_URL: 'https://evil.example/portal',
  }), /PORTAL/);
  const env = configuration();
  env.PUBLIC_CARE_PRO_PAYMENT_LINK = env.PUBLIC_CARE_BASIC_PAYMENT_LINK;
